...
struct multithreading_loop_unrolling_thread_args {
    // the range that each thread deals with [start, end)
    int start, end;
        ...
        for (int col = mat_args->start; col < mat_args->end; col += 4) {
            ...
            for (int ch = 0; ch < k;) {
                ...
                for (int qj = 0; qj < 16; qj++) {
                    ...
                    // need to do accumulate addition for A[row][ch] * B[col][ch]
                    // 1 packed_byte refer to which ouput?
                    /*
                    for intermediate_sum0 (calculate C[row][col]):
                    - read weight from the col of B
                    - read int4 weight(packed_byte) from w0_int4
                    - unpacked and get 2 int4 value
                    - multiply and add with element of A accordingly
                    */
                    uint8_t packed_byte = w0_int4[qj];
                    int8_t w_low = packed_byte & 0x0F;
                    int8_t w_high = packed_byte >> 4;

                    w_low = (int8_t)(w_low << 4) >> 4;
                    w_high = (int8_t)(w_high << 4) >> 4;

                    // TODO: int8 multiply and accumulate operation
                    // a_int8[qj] = A[row][ch+qj]
                    int8_t a0 = a_int8[qj];
                    int8_t a1 = a_int8[16+qj];
                    intermediate_sum0 += w_high * a0;
                    intermediate_sum0 += w_low * a1;

                    uint8_t packed_byte1 = w1_int4[qj];
                    int8_t w1_low = packed_byte1 & 0x0F;
                    int8_t w1_high = packed_byte1 >> 4;
                    w1_low = (int8_t)(w1_low << 4) >> 4;
                    w1_high = (int8_t)(w1_high << 4) >> 4;
                    intermediate_sum1 += w1_high * a0;
                    intermediate_sum1 += w1_low * a1;

                    uint8_t packed_byte2 = w2_int4[qj];
                    int8_t w2_low = packed_byte2 & 0x0F;
                    int8_t w2_high = packed_byte2 >> 4;
                    w2_low = (int8_t)(w2_low << 4) >> 4;
                    w2_high = (int8_t)(w2_high << 4) >> 4;
                    intermediate_sum2 += w2_high * a0;
                    intermediate_sum2 += w2_low * a1;

                    uint8_t packed_byte3 = w3_int4[qj];
                    int8_t w3_low = packed_byte3 & 0x0F;
                    int8_t w3_high = packed_byte3 >> 4;
                    w3_low = (int8_t)(w3_low << 4) >> 4;
                    w3_high = (int8_t)(w3_high << 4) >> 4;
                    intermediate_sum3 += w3_high * a0;
                    intermediate_sum3 += w3_low * a1;
                ...
                for (int qj = 0; qj < 32; qj++) {
                    ...
                    if (qj < 16) {
                        ...
                        uint8_t pb1 = w1_int4[qj];
                        int8_t w1_low = (int8_t)((pb1 & 0x0F) << 4) >> 4;
                        int8_t w1_high = (int8_t)((pb1 >> 4) << 4) >> 4;
                        intermediate_sum1 += w1_high * a0 + w1_low * a1;

                        uint8_t pb2 = w2_int4[qj];
                        int8_t w2_low = (int8_t)((pb2 & 0x0F) << 4) >> 4;
                        int8_t w2_high = (int8_t)((pb2 >> 4) << 4) >> 4;
                        intermediate_sum2 += w2_high * a0 + w2_low * a1;

                        uint8_t pb3 = w3_int4[qj];
                        int8_t w3_low = (int8_t)((pb3 & 0x0F) << 4) >> 4;
                        int8_t w3_high = (int8_t)((pb3 >> 4) << 4) >> 4;
                        intermediate_sum3 += w3_high * a0 + w3_low * a1;
                    } else {
                        uint8_t pb0 = w0_int4[qj];
                        int8_t w0_low = (int8_t)((pb0 & 0x0F) << 4) >> 4;
                        int8_t w0_high = (int8_t)((pb0 >> 4) << 4) >> 4;
                        intermediate_sum0_2nd += w0_high * a0 + w0_low * a1;

                        uint8_t pb1 = w1_int4[qj];
                        int8_t w1_low = (int8_t)((pb1 & 0x0F) << 4) >> 4;
                        int8_t w1_high = (int8_t)((pb1 >> 4) << 4) >> 4;
                        intermediate_sum1_2nd += w1_high * a0 + w1_low * a1;

                        uint8_t pb2 = w2_int4[qj];
                        int8_t w2_low = (int8_t)((pb2 & 0x0F) << 4) >> 4;
                        int8_t w2_high = (int8_t)((pb2 >> 4) << 4) >> 4;
                        intermediate_sum2_2nd += w2_high * a0 + w2_low * a1;

                        uint8_t pb3 = w3_int4[qj];
                        int8_t w3_low = (int8_t)((pb3 & 0x0F) << 4) >> 4;
                        int8_t w3_high = (int8_t)((pb3 >> 4) << 4) >> 4;
                        intermediate_sum3_2nd += w3_high * a0 + w3_low * a1;
                    }
                    // TODO: int8 multiply and accumulate operation
                }
                // dequantize the sum into floating point
